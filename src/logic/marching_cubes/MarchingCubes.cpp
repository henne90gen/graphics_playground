...
glm::vec3 getEdgeCenter(glm::vec4 v1, glm::vec4 v2) {
#if 1
...
void runMarchingCubes(const glm::ivec3 &dimensions, std::vector<glm::vec3> &vertices, std::vector<glm::ivec3> &indices,
    ...
    const unsigned int width = dimensions.x;
    const unsigned int height = dimensions.y;
    const unsigned int depth = dimensions.z;
    ...
    vertices.reserve(width * height * depth * maxTrianglesPerCube * 3);
    indices.reserve(width * height * depth * maxTrianglesPerCube);

    while (true) {
        // float cubeCenterValue = func(cubePosition.x, cubePosition.y, cubePosition.z);
        ...
        for (unsigned int i = 0; triangulation[cubeIndex][i] != -1; i += 3) {
            ...
            const glm::vec3 v0 = getEdgeCenter(glm::vec4(cubeCorners[a0], surfaceValues[a0]),
                                               glm::vec4(cubeCorners[b0], surfaceValues[b0]));
            const glm::vec3 v1 = getEdgeCenter(glm::vec4(cubeCorners[a1], surfaceValues[a1]),
                                               glm::vec4(cubeCorners[b1], surfaceValues[b1]));
            const glm::vec3 v2 = getEdgeCenter(glm::vec4(cubeCorners[a2], surfaceValues[a2]),
                                               glm::vec4(cubeCorners[b2], surfaceValues[b2]));
