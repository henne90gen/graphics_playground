...
void AmbientOcclusion::setup() {
    ...
    cube1 = createCubeVA(shader);
    cube2 = createCubeVA(shader);
...
void AmbientOcclusion::tick() {
    ...
    cube1->bind();
    GL_Call(glDrawElements(GL_TRIANGLES, cube1->getIndexBuffer()->getCount(), GL_UNSIGNED_INT, nullptr));

    auto modelMatrix2 = createModelMatrix(position2);
    shader->setUniform("modelMatrix", modelMatrix2);
    cube2->bind();
    GL_Call(glDrawElements(GL_TRIANGLES, cube2->getIndexBuffer()->getCount(), GL_UNSIGNED_INT, nullptr));
    ...
    const glm::vec3 scale = glm::vec3(getAspectRatio() * 1.5F, 2.0F, 1.0F);
