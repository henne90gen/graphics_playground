...
float hashrand(int i) { return (float)(position_hash(std::to_string(i)) % 1000) / 1000.0f; }
...
struct Branch {
    ...
    // Constructors
    Branch(float r, float s, float ss) : ratio{r}, spread{s}, splitsize{ss} {};

    Branch(Branch *b, bool root) : ratio{b->ratio}, spread{b->spread}, splitsize{b->splitsize} {
        if (root)
            return;
        depth = b->depth + 1;
        P = b; // Set Parent
...
void constructTree(Branch *root, std::vector<glm::vec3> &positions, std::vector<glm::vec3> &normals,
    ...
    std::function<void(Branch *, glm::vec3)> addBranch = [&](Branch *b, glm::vec3 p) {
        ...
        glm::mat4 r = glm::rotate(glm::mat4(1.0), glm::pi<float>() / ringsize, b->dir);
        ...
        for (int i = 0; i < ringsize; i++) {
            positions.push_back(start + b->radius * treescale[1] * glm::vec3(n));
            normals.emplace_back(n);
            n = r * n;

            positions.push_back(end + taper * b->radius * treescale[1] * glm::vec3(n));
            normals.emplace_back(n);
            n = r * n;
        }
...
void addLeaves(Branch *root, std::vector<glm::mat4> &p, bool face) {
    p.clear();
    ...
    std::function<void(Branch *, glm::vec3)> addLeaf = [&](Branch *b, glm::vec3 pos) {
        ...
        for (int i = 0; i < leafcount; i++) {
            ...
            glm::vec3 d =
                  glm::vec3(hashrand(b->ID + i), hashrand(b->ID + i + leafcount), hashrand(b->ID + i + 2 * leafcount)) -
                  glm::vec3(0.5);
